...
# 1. Check if .ado.env exists (look in parent directory)
if [ ! -f "../.ado.env" ]; then
...
if [ -n "$SERVICE_ENDPOINT_ID" ] && [ "$SERVICE_ENDPOINT_ID" != "null" ]; then
    ...
    if [ -n "$SP_ID" ]; then
        ...
        if [ -n "$SP_OBJECT_ID" ]; then
            ...
            # Update terraform.tfvars with the service principal object ID
            TFVARS_FILE="../terraform.tfvars"
            if [ -f "$TFVARS_FILE" ]; then
                echo "Updating $TFVARS_FILE with azure_devops_sp_object_id..."
                ...
                echo -e "${GREEN}✅ terraform.tfvars updated with Service Principal Object ID.${NC}"
                ...
                if terraform apply -auto-approve >/dev/null 2>&1; then
                    ...
                    echo -e "${YELLOW}⚠️ Terraform apply had warnings. Checking if access policy was set...${NC}"
